...
tt
nod* multiset<T>::insert(T key , nod* p){
    if(p == NULL){
        p = new nod;
        p->data = key;
        p->count = 1;
...
tt
int multiset<T>::balance_factor(nod* p){
    int hl,hr;
...
tt
nod* multiset<T>::LLrotation(nod* p){
    nod* temp = p->left;
    p->left = temp->right;
    temp->right = p;
    p->height = height(p);
...
tt
nod* multiset<T>::LRrotation(nod* p){
...
tt
nod* multiset<T>::RRrotation(nod* p){
    nod* temp = p->right;
    p->right = temp->left;
    temp->left = p;
    p->height = height(p);
...
tt
nod* multiset<T>::RLrotation(nod* p){
...
tt
void multiset<T>::preorderTraverse(nod* p){
    if(p == NULL)return;
...
tt
void multiset<T>::delete_tree(nod* p){
    if(p == NULL)return;
    delete_tree(p->left);
    delete_tree(p->right);
    delete p;
